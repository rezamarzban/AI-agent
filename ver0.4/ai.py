...
TOOL_INTERACTIONS = []   # Global log for Tools Interactions tab
...
def stream_model(messages, send_func=None):
    ...
    if send_func:
        send_func("data: Connecting to server...\n\n")
    else:
...
def process_turn(history, send_func=None):
    ...
    while step < max_steps:
        ...
        for tc in calls:
            ...
            # Save full interaction for Tools Interactions tab / CLI log
...
def test_single_tool():
    print("\n=== Tool Tester ===")
    print("Available:", list(TOOLS.keys()))
    name = input("Tool name (empty to cancel): ").strip()
    if not name or name not in TOOLS:
        return
    args_str = input("Args as JSON (empty = {}): ").strip()
    try:
...
def run_cli():
    ...
    print("Commands: 'test' = tool tester, 'tools' = show interactions log, 'exit' = quit\n")
...
if __name__ == "__main__":
    print("Choose mode:")
    print("1. CLI mode")
    print("2. Web mode (with tabs: Chat / Tools Interactions / Tool Testing)")
