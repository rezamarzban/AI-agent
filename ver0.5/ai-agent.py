...
def stream_model(messages, send_func=None, send_status=True):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            time.sleep(wait)
            if not send_func and send_status:
    ...
    # Fallback: If no tool_calls but content looks like tool calls, parse it
    if not msg.get("tool_calls") and accumulated_content and re.search(r'\{"type":\s*"function"', accumulated_content):
        try:
            ...
            for i, part in enumerate(parts):
                ...
                if 'type' in parsed and parsed['type'] == 'function' and 'name' in parsed and 'arguments' in parsed:
                    clean_calls.append({
                        "id": f"call_{i}",
                        "type": "function",
                        "function": {
            ...
            if clean_calls:
                ...
                msg["content"] = None  # Clear content as it's tool calls
                report = "🔧 [Fallback: Parsed tool calls from content]"
                if send_func:
                    send_func(f"data: {report}\n\n")
                else:
                    print(report)
        except Exception as e:
            pass  # Silently ignore if parsing fails, no report
